...
def get_user_friends():
	...
	user_id = get_user_id()
	cursor = conn.cursor()
	print cursor.execute("SELECT u.user_id, u.first_name, u.last_name FROM Users AS u, Friends AS f WHERE f.friend_id = '{0}' AND u.user_id = f.user_id".format(user_id))
	friends +=cursor.fetchall()
	
	return friends # [(user_id, first_name, last_name)]
...
def get_active_users():
	...
	cursor = conn.cursor()
	print cursor.execute("SELECT first_name, contribution FROM Users ORDER BY contribution DESC LIMIT 10")
...
def register():
	return render_template('improved_register.html')  
...
def register_user():
	...
	if test:
		...
		return render_template('improved_register.html', message='Email already in use! Use another one or ')
...
def get_photos_by_most_used_tags(mostUsedTags):
	print "get_photos_by_most_used_tags"
	photos = ()
	for tag in mostUsedTags:
		cursor = conn.cursor()
		print tag[0]
		print cursor.execute("SELECT data, Photos.photo_id, caption, album_id FROM Photos,Associate where Associate.word='{0}' and Associate.photo_id=Photos.photo_id".format(tag[0]))
		photos+= cursor.fetchall() 
	return photos #NOTE list of tuples, [(data, photo_id, caption, album_id), ...]

def get_most_used_tags():
	print "get most used tags"
	user_id = get_user_id()
	cursor = conn.cursor()
	print cursor.execute("SELECT ac.word FROM Albums AS a, Photos AS p, Associate AS ac WHERE a.user_id = '{0}' AND p.album_id = a.album_id AND ac.photo_id = p.photo_id group by ac.word order by COUNT(ac.word) limit 5".format(user_id)  )
	return cursor.fetchall()

@app.route("/recommendation", methods = ['GET'])
@flask_login.login_required
def recommendation():
	mostUsedTags = get_most_used_tags()
	print mostUsedTags
	photos = get_photos_by_most_used_tags(mostUsedTags)
...
def show_all_photos():
	all_photos = get_all_photos()
	return render_template('all_photos.html', photos=all_photos)
